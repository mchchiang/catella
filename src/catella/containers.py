...
class DataFrameMap(MutableMapping):
    ...
    def __setitem__(self, key: str, value: Any) -> None:
        ...
        if not isinstance(value, (pd.DataFrame, H5Array)):
            raise TypeError(
                f"Value for key '{key}' must be a pandas DataFrame or H5Array"
            )
...
class FixedKeyMap(MutableMapping):
    ...
    def __setitem__(self, key: str, value: Any) -> None:
        if key not in self._data:
            raise KeyError(
                f"Key '{key}' is not predefined. New keys are not allowed."
            )
        self._data[key] = value
...
class DataclassPublicProxy(Mapping):
    def __init__(self, obj):
        ...
        self._keys = [
            f.name for f in fields(obj) if not f.name.startswith("_")
        ]
    ...
    def __repr__(self) -> str:
        ...
        items = [
            f"{f.name}={getattr(self._obj, f.name)!r}"
